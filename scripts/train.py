...
def train_one_epoch(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, tb_writer=None):
    ...
    if args.method != 'base':  # load unlabeled images
    ...
    for i, batch in enumerate(dataloader):
        ...
        if args.method != "base": # use unlabeled images
            ...
            if 'keyword' in data:
                keyword_labels = batch[2]
                keyword_labels = keyword_labels.to(device=device, non_blocking=True)
                model_out_extra = {'keyword_labels': keyword_labels}

                keywords = data['keyword']
                keywords = keywords.to(device=device, non_blocking=True)
                inputs.update({'keyword': keywords})
        ...
        if args.accum_freq == 1:
            with autocast():
                ...
                model_out.update(model_out_extra)
